...
class FERPlusDataset(Dataset):
    ...
    def __init__(self, root_dir, split='train', image_size=(224, 224), transform_type=None):
        ...
        normalization_path = os.path.join(root_dir, 'normalization_params.npz')
        loaded = np.load(normalization_path)
        self.global_mean = float(loaded['global_mean'])
        self.global_std = float(loaded['global_std'])
    ...
    def normalize_sparse_heatmap(heatmap):
        """Normalize sparse heatmap while preserving signal."""
        if heatmap.max() <= 1e-8:
            return heatmap
    ...
    def __getitem__(self, idx):
        ...
        # Normalize and transform heatmap
        ...
        if self.transform_type == 'train':
            # Ensure both tensors are 4D (batch, channel, height, width)
            # transformed_image = transformed_image.unsqueeze(0)
            # transformed_heatmap = transformed_heatmap.unsqueeze(0)
            # print("Image shape:", transformed_image.shape)
            # print("Heatmap shape:", transformed_heatmap.shape)
            ...
            #transformed_image = four_channel[0:1, :, :]
            #transformed_heatmap = four_channel[1:, :, :]
...
def visualize_heatmaps(train_loader, num_samples=5, output_path="./outputs/heatmap_visualization.png"):
    ...
    batch = next(dataiter)
    # Unpack the batch
    images, labels = batch  # images: [B, 2, 224, 224] - giờ là 2 channels
    ...
    for i in range(num_vis):
        ...
        axes[i][0].set_title(f"Grayscale (Label: {EMOTION_MAPPING.get(lbl + 1, lbl + 1)})")
        axes[i][0].axis('off')
        # Plot heatmap
...
def modified_manet_with_pretrain(num_classes=7):
    ...
    new_conv = nn.Conv2d(
        2,  # 2 kênh đầu vào (grayscale + heatmap)
        64,  # Giữ nguyên số kênh đầu ra
        kernel_size=7,
        stride=2,
        padding=3,
    ...
    with torch.no_grad():
        ...
        # Khởi tạo trọng số kênh heatmap là giá trị trung bình của trọng số RGB
        new_conv.weight[:, 1:2] = original_conv.weight.mean(dim=1, keepdim=True)
    # Thay thế lớp conv ban đầu
    model.conv1 = new_conv
    return model
...
def main():
    ...
    train_dataset = FERPlusDataset(args.data, split='train', transform_type='train')
